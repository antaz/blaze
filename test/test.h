...
#define assert_eq(X, Y)                                                        \
	do {                                                                   \
		...
		if (t__ret_x != t__ret_y)                                      \
			test_failf(#X " expected '%d', got '%d'", t__ret_y,    \
				   t__ret_x);                                  \
		else                                                           \
			test_pass();                                           \
	} while (0)
...
#define assert_lt(X, Y)                                                        \
	do {                                                                   \
		...
		int t__ret_y = (Y);                                            \
		if (!(t__ret_x < t__ret_y))                                    \
			test_failf(#X " expected '%d' to be less than '%d'",   \
				   t__ret_y, t__ret_x);                        \
		else                                                           \
			test_pass();                                           \
	} while (0)
...
#define assert_ueq(X, Y)                                                       \
	do {                                                                   \
		...
		unsigned t__ret_y = (Y);                                       \
		if (t__ret_x != t__ret_y)                                      \
			test_failf(#X " expected '%u', got '%u'", t__ret_y,    \
				   t__ret_x);                                  \
		else                                                           \
			test_pass();                                           \
	} while (0)
...
#define assert_strncmp(X, Y, N)                                                \
	do {                                                                   \
		...
		if (t__assert_strncmp_(t__ret_x, t__ret_y, (N)))               \
			test_failf(                                            \
			    #X " expected '%.*s', got '%.*s'", (int)(N),       \
			    t__ret_y == NULL ? "NULL" : t__ret_y, (int)(N),    \
			    t__ret_x == NULL ? "NULL" : t__ret_x);             \
		else                                                           \
			test_pass();                                           \
	} while (0)
...
#define assert_ptr_eq(X, Y)                                                    \
	do {                                                                   \
		...
		if (t__ret_x != t__ret_y)                                      \
			test_failf(#X " expected '%016" PRIxPTR                \
				      "', got '%016" PRIxPTR "'",              \
				   t__ret_y, t__ret_x);                        \
		else                                                           \
			test_pass();                                           \
...
#define assert_ptr_null(X)                                                     \
	do {                                                                   \
		uintptr_t t__ret_x = (uintptr_t)(X);                           \
		if (t__ret_x)                                                  \
			test_failf(#X " expected NULL, got '%016" PRIxPTR "'", \
				   t__ret_x);                                  \
		else                                                           \
			test_pass();                                           \
	} while (0)
 ...
 *  - in normal operation, fatal errors abort program execution by calling
 * exit().
 *  - in testcases however, fatal errors are considered testcase failures and
 * jump to one of two places:
...
#define t__fatal(...)                                                          \
	do {                                                                   \
		if (t__tc_assert_fatal_) {                                     \
			...
			snprintf(t__tc_errbuf_1_, sizeof(t__tc_errbuf_1_),     \
				 "%s:%u:%s:", __FILE__, __LINE__, __func__);   \
			snprintf(t__tc_errbuf_2_, sizeof(t__tc_errbuf_2_),     \
				 __VA_ARGS__);                                 \
...
static inline int t__assert_strcmp_(const char *p1, const char *p2)
{
	if (!p1 || !p2)
		return p1 != p2;
...
static inline int t__assert_strncmp_(const char *p1, const char *p2, size_t n)
{
	if (!p1 || !p2)
		return p1 != p2;
...
static int t__run_tests_(int (*tc_init)(void), int (*tc_term)(void),
			 const struct testcase testcases[], const size_t len)
{
