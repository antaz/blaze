...
#define assert_gt(X, Y)                                                        \
    do {                                                                       \
        ...
        int t__ret_y = (Y);                                                    \
        if (!(t__ret_x > t__ret_y))                                            \
            test_failf(#X " expected '%d' to be greater than '%d'", t__ret_y,  \
                       t__ret_x);                                              \
        else                                                                   \
            test_pass();                                                       \
    } while (0)
...
#define assert_lt(X, Y)                                                        \
    do {                                                                       \
        ...
        int t__ret_y = (Y);                                                    \
        if (!(t__ret_x < t__ret_y))                                            \
            test_failf(#X " expected '%d' to be less than '%d'", t__ret_y,     \
                       t__ret_x);                                              \
        else                                                                   \
            test_pass();                                                       \
    } while (0)
...
#define assert_strncmp(X, Y, N)                                                \
    do {                                                                       \
        ...
        if (t__assert_strncmp_(t__ret_x, t__ret_y, (N)))                       \
            test_failf(#X " expected '%.*s', got '%.*s'", (int)(N),            \
...
#define assert_ptr_eq(X, Y)                                                    \
    do {                                                                       \
        ...
        if (t__ret_x != t__ret_y)                                              \
            test_failf(#X " expected '%016" PRIxPTR "', got '%016" PRIxPTR     \
                          "'",                                                 \
                       t__ret_y, t__ret_x);                                    \
        else                                                                   \
            test_pass();                                                       \
