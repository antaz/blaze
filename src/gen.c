...
uint64_t pin(const uint64_t bb[], const int color)
{
        ...
        pinner = ratk(k, bb[!color]) &
                  ((bb[ROOK] | bb[QUEEN]) & bb[!color]);
        pinner |= batk(k, bb[!color]) &
                  ((bb[BISHOP] | bb[QUEEN]) & bb[!color]);
        while (pinner) {
...
void generate(const struct board_t *board, uint16_t *moves)
{
	...
	uint64_t quiet;
		...
		case KNIGHT:
			...
			while(target) {
				*moves++ = move(bsf(target), bsf(checker), CAPTURE);
				target &= target - 1;
			}
			break;
		default:
