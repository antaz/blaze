...
void addPawnMove(Board *board, MoveList *list, unsigned char from, unsigned char to, unsigned char captured, unsigned char enPassant, unsigned char pawnStart, unsigned char castle) {
	...
	if(board->pieces[from] == wP && RANK120(from) == RANK_7) {
		addMove(board, list, from, to, captured, wQ, enPassant, pawnStart, castle);
		addMove(board, list, from, to, captured, wR, enPassant, pawnStart, castle);
		addMove(board, list, from, to, captured, wB, enPassant, pawnStart, castle);
		addMove(board, list, from, to, captured, wN, enPassant, pawnStart, castle);	
	} else if(board->pieces[from] == bP && RANK120(from) == RANK_2) {
		addMove(board, list, from, to, captured, bQ, enPassant, pawnStart, castle);
		addMove(board, list, from, to, captured, bR, enPassant, pawnStart, castle);
		addMove(board, list, from, to, captured, bB, enPassant, pawnStart, castle);
		addMove(board, list, from, to, captured, bN, enPassant, pawnStart, castle);
	} else {
		addMove(board, list, from, to, captured, EMPTY, enPassant, pawnStart, castle);
	}
}
		...
		for(pieceCount = 0; pieceCount < board->pieceCount[wP]; ++pieceCount) {
			...
			if(board->pieces[square + 10] == EMPTY) {
				...
				if(RANK120(square) == RANK_2 && board->pieces[square + 20] == EMPTY) {
					addMove(board, list, square, square + 20, EMPTY, EMPTY, 0, 1, 0);
				}
			}
			...
			if(board->enPassant != NO_SQ) {
				if(square + 9 == board->enPassant) {
					addMove(board, list, square, square + 9, EMPTY, EMPTY, 1, 0, 0);
				}
				if(square + 11 == board->enPassant) {
					addMove(board, list, square, square + 11, EMPTY, EMPTY, 1, 0, 0);
				}
			}
		}
		...
		if(board->castling & WKCA) {
			if(board->pieces[F1] == EMPTY && board->pieces[G1] == EMPTY) {
				if(!isAttacked(board, E1, BLACK) && !isAttacked(board, F1, BLACK)) {
					addMove(board, list, E1, G1, EMPTY, EMPTY, 0, 0, 1);
				}
			}
		}		
		...
		if(board->castling & WQCA) {
			if(board->pieces[D1] == EMPTY && board->pieces[C1] == EMPTY && board->pieces[B1] == EMPTY) {
				if(!isAttacked(board, E1, BLACK) && !isAttacked(board, D1, BLACK)) {
					addMove(board, list, E1, C1, EMPTY, EMPTY, 0, 0, 1);
				}
			}
		}
	} else {
		...
		if(board->castling & BKCA) {
			if(board->pieces[F8] == EMPTY && board->pieces[G8] == EMPTY) {
				if(!isAttacked(board, E8, WHITE) && !isAttacked(board, F8, WHITE)) {
					addMove(board, list, E8, G8, EMPTY, EMPTY, 0, 0, 1);
				}
			}
		}		
		...
		if(board->castling & BQCA) {
			if(board->pieces[D8] == EMPTY && board->pieces[C8] == EMPTY && board->pieces[B8] == EMPTY) {
				if(!isAttacked(board, E8, WHITE) && !isAttacked(board, D8, WHITE)) {
					addMove(board, list, E8, C8, EMPTY, EMPTY, 0, 0, 1);
				}
			}
		}
	}
		...
		for(pieceCount = 0; pieceCount < board->pieceCount[piece]; pieceCount++) {
			...
			for(index = 0; index < numDirs[piece]; index++) {
				...
				while(board->pieces[targetSquare] != OFFBOARD) {
					...
					addMove(board, list, square, targetSquare, EMPTY, EMPTY, 0, 0, 0);
					targetSquare += dir;
				}
			}	
		}
