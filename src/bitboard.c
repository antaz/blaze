...
static const uint64_t _FILE[8] = {0x101010101010101,  0x202020202020202,
...
static const uint64_t KATTACK[64] = {0x302,
                                     0x705,
                                     0xe0a,
                                     0x1c14,
                                     0x3828,
...
static const uint64_t NATTACK[64] = {0x20400,
                                     0x50800,
                                     0xa1100,
                                     0x142200,
                                     0x284400,
...
static const uint64_t WPATTACK[64] = {0x200,
                                      0x500,
                                      0xa00,
                                      0x1400,
                                      0x2800,
...
static const uint64_t BPATTACK[64] = {0x0,
                                      0x0,
                                      0x0,
                                      0x0,
                                      0x0,
...
// Reverse bitboard
static uint64_t rev(uint64_t bb)
{
        ...
        return (bb << 48) | ((bb & 0xffff0000) << 16) |
               ((bb >> 16) & 0xffff0000) | (bb >> 48);
}

// Hyperbola Quintessence o^(o - 2r)
static uint64_t oo2r(int i, uint64_t block, uint64_t mask)
{
        ...
        for (int i = 0; i < 64; ++i)
                for (int j = 0; j < 64; ++j) {
                        ...
                        if (file(i) == file(j) || rank(i) == rank(j))
                                BETWEEN[i][j] = rattack(i, sq) & rattack(j, sq);
                        else if (diag(i) == diag(j) || adiag(i) == adiag(j))
                                BETWEEN[i][j] = battack(i, sq) & battack(j, sq);
                }
}
...
int bsf(uint64_t bb) { return popcount((bb & -bb) - 1); }

uint64_t kattack(int i) { return KATTACK[i]; }

uint64_t nattack(int i) { return NATTACK[i]; }

uint64_t pattack(int i, bool color)
{
        return color ? WPATTACK[i] : BPATTACK[i];
}
...
uint64_t qattack(int i, uint64_t block)
{
        return battack(i, block) | rattack(i, block);
}
...
uint64_t between(int i, int j)
{
        ...
        if (file(i) == file(j) || rank(i) == rank(j))
                return rattack(i, sq) & rattack(j, sq);
        else if (diag(i) == diag(j) || adiag(i) == adiag(j))
                return battack(i, sq) & battack(j, sq);
        else
                return 0ULL;
}
...
uint64_t line(int i, int j)
{
        // return LINE[i][j];
        if (file(i) == file(j) || rank(i) == rank(j))
                return (rattack(i, 0ULL) & rattack(j, 0ULL)) | (1ULL << i) |
                       (1ULL << j);
        else if (diag(i) == diag(j) || adiag(i) == adiag(j))
                return (battack(i, 0ULL) & battack(j, 0ULL)) | (1ULL << i) |
                       (1ULL << j);
        else
                return 0ULL;
}
